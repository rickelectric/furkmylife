...
public class LoginWindow extends AppFrameClass {
	...
	public LoginWindow() {
		...
		lblhelp.setIcon(new ImageIcon(LoginWindow.class
				.getResource("/rickelectric/furkmanager/img/dash/User-16.png")));
		lblhelp.setBorder(new BevelBorder(BevelBorder.RAISED, null, null, null,
				null));
		lblhelp.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				// TODO Open Help On Finding API Key On Furk.net With Link
				// TODO Make a Frame with clickable links to furk.net and
				// paginated images hilighting steps.
		...
		check_remember = new JCheckBox("Remember Me");
		check_remember.setBounds(7, 290, 162, 20);
		contentPane.add(check_remember);

		check_autologin = new JCheckBox("Enable Auto Login");
		...
		label_logo.setIcon(new ImageIcon(FurkManager.class
				.getResource("img/fl_anim/fr-logo-speed-0.gif")));
	...
	private void login(){
		...
		userLogin.autoLogin(check_autologin.isSelected());
		
		boolean loggedIn=API_UserData.login(userLogin);
		if(loggedIn){
			//TODO Invoke Main Window & End Thread
			FurkManager.appRun();
		}
	}
	...
	private void apiLogin(){
		...
		apiLogin.save(check_remember.isSelected());
		apiLogin.autoLogin(check_autologin.isSelected());
	...
	Runnable apiLogin = new Runnable() {
		...
		public void run() {
			...
			try {
				...
				if (keyPing) {
					boolean apiRem = check_remember.isSelected();
					SettingsManager.setApiRemember(apiRem);
					if (apiRem) {
		...
		public void run() {
			...
			try {
				...
				if (login) {
					...
					if (userRem) {
						SettingsManager.setLogin(username, password);
						SettingsManager.save();
					} else {
						SettingsManager.setLogin("", "");
					}
					dispose();
					loggingIn = false;
					FurkManager.appRun();
	...
	private JCheckBox check_remember;
	private JCheckBox check_autologin;
	private JCheckBox check_savecreds;
	
	/**
	 * Animate The Furk Logo by changing the static image to one of the animated
	 * gifs.
	 * 
	 * @param speed
	 *            Speed of the animation. A value between 0(still) and
	 *            4(fastest) inclusive.
	 */
	public void furkAnimate(int speed) {
		if (speed >= 0 && speed <= 4)
			label_logo
					.setIcon(new ImageIcon(FurkManager.class
							.getResource("img/fl_anim/fr-logo-speed-" + speed
									+ ".gif")));
	}
