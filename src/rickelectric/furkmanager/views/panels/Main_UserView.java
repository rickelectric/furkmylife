...
public class Main_UserView extends TranslucentPane {
	...
	public Main_UserView(){
		...
		panel_userdetails.setBorder(new TitledBorder(null, "User Info", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		if(FurkUserData.User.emailVerified==0) pane_info.add(button_VerifyEmail);
		
		loading_box_tab1 = new JLabel("");
		loading_box_tab1.setBounds(457, 274, 80, 80);
		loading_box_tab1.setIcon(new ImageIcon(FurkManager.class.getResource("img/ajax-loader.gif")));
		loading_box_tab1.setVisible(false);
		pane_info.add(loading_box_tab1);
		
		JPanel pane_bandwidth = new JPanel();
		tabbedPane.addTab("Bandwidth Stats", null, pane_bandwidth, null);
		pane_bandwidth.setLayout(null);
		
		JPanel panel_searchdl = new JPanel();
		...
		loading_box_tab3.setIcon(new ImageIcon(FurkManager.class.getResource("img/ajax-loader.gif")));
		...
		saving_box_tab_3.setIcon(new ImageIcon(FurkManager.class.getResource("img/ajax-loader.gif")));
		saving_box_tab_3.setVisible(false);
		panel_searchdl.add(saving_box_tab_3);
		button_save.setBounds(12, 328, 131, 26);
		panel_searchdl.add(button_save);
		
		JPanel panel_dlinks = new JPanel();
		panel_dlinks.setOpaque(false);
		panel_dlinks.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Direct Download Links", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		choice_scheme.setModel(new DefaultComboBoxModel<String>(URI_Enums.URI_Scheme.stringValues()));
		...
		choice_host.setModel(new DefaultComboBoxModel<String>(URI_Enums.URI_Host.stringValues()));
		
		choice_port = new JComboBox<String>();
		choice_port.setBounds(91, 58, 160, 25);
		panel_dlinks.add(choice_port);
		choice_port.setModel(new DefaultComboBoxModel<String>(URI_Enums.URI_Port.stringValues()));
		...
		spinner_linkkey.setToolTipText("Premium Feature: \r\nTo reset all your direct download links, change this number to any other positive number. '0' disables this function.");
		...
		panel_search.setBorder(new TitledBorder(new LineBorder(new Color(184, 207, 229)), "Search", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		...
		Thread t=new Thread(new Runnable(){
			...
			public void run(){
				try{
					...
					check_noadult.setSelected(FurkUserData.User.flags==URI_Enums.Prefs_Flags.MODERATED_FULL);
					choice_host.setSelectedItem(URI_Enums.URI_Host.getName(FurkUserData.User.dlHost));
					choice_port.setSelectedItem(URI_Enums.URI_Port.getName(FurkUserData.User.dlPort));
					choice_scheme.setSelectedItem(URI_Enums.URI_Scheme.getName(FurkUserData.User.dlScheme));
