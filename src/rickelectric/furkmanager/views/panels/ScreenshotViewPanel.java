...
public class ScreenshotViewPanel extends JPanel {
	...
	public ScreenshotViewPanel(FurkTFile tf) {
		...
		tf.getThumbURL();
		try {
			BufferedImage img = StreamDownloader.getInstance().getImageStream(
					tf.getThumbURL(), 8);
			...
			for (int i = 0; i < 9; i++) {
				...
				RequestCache.ImageR.add(imgloc[i],
						img.getSubimage(width * i, 0, width, height));
			}
			...
			JOptionPane.showMessageDialog(null, "Failed to show screenshots.",
	...
	private void init(final String[] ssthumbs, final String[] ssfull) {
		setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED, null,
				null, null, null), "Screenshots", TitledBorder.LEADING,
		...
		button_prev.setIcon(new ImageIcon(ImageLoader.getInstance().getImage(
		...
		button_next.setIcon(new ImageIcon(ImageLoader.getInstance().getImage(
	...
	private void loadScreenshots(final String[] ssthumbs,
			final String[] ssfull, final int offset) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				for (int z = offset; z < offset + 3; z++) {
					if (ssf[z] == null) {
						ssf[z] = new ScreenshotIcon(ssthumbs[z], ssfull[z],
								z + 1, diffSizes);
					}
...
class ScreenshotIcon extends JLabel {
	...
	public ScreenshotIcon(String thumbUrl, String fullUrl, int num,
			boolean diffSizes) {
		super();
		ssNum = num;
		ajax = new ImageIcon(ImageLoader.class.getResource("ajax-loader.gif"));
		setIcon(ajax);
		ssThumbUrl = thumbUrl;
	...
	public void loadImg() {
		if (ssThumbUrl == null)
			return;
		try {
			BufferedImage ci = StreamDownloader.getInstance().getImageStream(
					ssThumbUrl, 8);
			float h = 90;
			...
			this.setIcon(new ImageIcon(ImageLoader.getInstance().getImage("remove.png")));
			repaint();
		}
	}
}
