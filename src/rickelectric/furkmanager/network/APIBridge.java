...
public class APIBridge{
	...
	public static void overrideCache(boolean o){overrideCache=o;}
	
	public static final int
		DL_ADD_URL=100,
		DL_ADD_INFO_HASH=101,
		DL_ADD_TORRENT=102,
	...
	public static String dlAdd(int type,String link){
		...
		if(type==DL_ADD_URL) dest+="&url=";
		else if(type==DL_ADD_INFO_HASH) dest+="&info_hash=";
			...
			try{
				...
				String stream=StreamDownloader.postDataPartStream(API_BASE+"/dl/add",parts);
				return stream;
			}catch(Exception e){
				e.printStackTrace();
	 ...
	 */
	public static String dlGet(int get,String param,int[] lim_ofs,boolean checkCache){
		if(api_key==null) return null;
	...
	public static String fileInfo(String info_hash){
		...
		String dest=API_BASE+"/file/info?"+key()+"&info_hash="+info_hash;
	...
	public static String TFileInfo(String fileID){
		if(api_key==null) return null;
		String dest=API_BASE+"/file/get?"+key()+"&id="+fileID+"&t_files=1";
		return jsonPost(dest,true,true);
	}
	
	public static String fileGet(int get,String param,int[] lim_ofs,String[] sort,boolean checkCache){
		if(api_key==null) return null;
		String dest=API_BASE+"/file/get?"+key();
		if(get==GET_ID) dest+="&id="+param;
		else if(get==GET_TYPE) dest+="&type="+param;
		else if(get==GET_TRASH) dest+="&unlinked=1";
		else if(get!=GET_ALL) throw new RuntimeException("Invalid File GET Type in parameter 1");
	...
	public static String fileLinkstate(int action,String[] ids){
		...
		switch(action){
			case LINK_LINK: destfile+="link";break;
			case LINK_UNLINK: destfile+="unlink";break;
			case LINK_CLEAR: destfile+="clear";break;
			default: throw new RuntimeException("Invalid Link Method");
		}
		String jsil="";
		for(int i=0;i<ids.length;i++){
			jsil+="id="+ids[i];
	...
	public static String labelAdd(String name){
		...
		try{name=URLEncoder.encode(name, "utf-8");}catch(Exception e){}
		dest+="&name="+name;
		return jsonPost(dest,false,false); 
	}
	
	public static String labelSave(String id,String name,String color,String sorder,String id_labels){
		if(id==null) return null;
		if(name==null&&color==null&&sorder==null&&id_labels==null) return null;
		String dest=API_BASE+"/label/upsert?"+key();
		dest+="&id="+id;
		if(name!=null){
			try{name=URLEncoder.encode(name, "utf-8");}catch(Exception e){}
		...
		if(color!=null){
			try{color=URLEncoder.encode(color, "utf-8");}catch(Exception e){}
			dest+="&color="+color;
		}
		if(sorder!=null){
			try{sorder=URLEncoder.encode(sorder, "utf-8");}catch(Exception e){}
			dest+="&sorder="+sorder;
		}
		if(id_labels!=null){
			try{id_labels=URLEncoder.encode(id_labels, "utf-8");}catch(Exception e){}
	...
	public static boolean userLogin(String username,String password){
		...
		if(username==null||password==null) throw new RuntimeException("Neither 'username' nor 'password' should be null.");
		try{username=URLEncoder.encode(username, "utf-8");}catch(Exception e){}
		try{password=URLEncoder.encode(password, "utf-8");}catch(Exception e){}
		Part[] params=new Part[]{
				new StringPart("login",username),
		...
		if(j.getString("status").equals("error")){
			throw new RuntimeException(j.getString("error")+" Tries Left: "+j.getString("tries_left"));
		}
	...
	private static String furkSearch(int sMode,String txt,boolean checkCache){
		...
		String frag="";
		try{txt=java.net.URLEncoder.encode(txt,"utf-8");}catch(Exception e){}
		if(sMode==0){frag="file/get";txt="&name_like="+txt;}
		if(sMode==1){frag="search";txt="&q="+txt;}
		else if(sMode==2){frag="plugins/metasearch";txt="&q="+txt;}
