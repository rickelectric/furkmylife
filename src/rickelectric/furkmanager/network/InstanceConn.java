...
/**
 * Ensures Only One Instance Of The Application Is Running At A Time Using Sockets. <br/>
...
public class InstanceConn{
	...
	public boolean appStart(){return appStart;}
	...
	public InstanceConn() throws InterruptedException{
		...
		cli.start();
		while(cli.isAlive());
		if(c.response().equals("accepted")) appStart=false;
		else if(c.response().equals("no-server")){
			s = new ServerLink();
			s.run();
			UtilBox.pause(2000);
			if(s.error()!=null&&s.error().equals("Port Error")){
				FurkManager.trayAlert(FurkManager.TRAY_ERROR,"Error",s.error(),null);
				appStart=false;
			}
			appStart=true;
		}
	...
	public boolean transmit(String s){
		if(!appStart){
			...
			cli.start();
			while(cli.isAlive());
			if(c.response().equals("accepted")) return true;
		}
		return false;
	}
